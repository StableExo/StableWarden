...
export const EntryCard: React.FC<EntryCardProps> = ({ entry }) => {
  ...
  return (
    <div className={`border-l-4 rounded-lg p-5 ${SIGNIFICANCE_STYLES[entry.significance]}`}>
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1">
          <div className="flex items-center gap-2 flex-wrap mb-1">
            ...
            <span className="text-xs text-base-content/60 font-mono flex items-center gap-1">
              <Hash size={10} className="opacity-70" />
              {entry.commitHash}
            </span>
          </div>
          <h3 className="text-lg font-bold text-base-content">{entry.title}</h3>
          <p className="text-sm text-base-content/70 mt-0.5">{entry.date}</p>
        </div>
        <button
      ...
      <p className="text-sm text-base-content/70 mt-3 font-mono bg-base-300/50 rounded px-3 py-2">
        {entry.description}
      </p>
      ...
      {expanded && (
        <div className="mt-4 space-y-4">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {entry.capabilities.map((cap, i) => (
                <span key={i} className="bg-base-200 rounded-full px-3 py-1 text-sm text-base-content/85 flex items-center gap-1.5">
                  <span>{cap.icon}</span>
                  {cap.label}
                </span>
              ))}
          ...
          <div className="border-t border-base-content/10 pt-4">
            ...
            <p className="text-sm text-base-content/85 italic leading-relaxed">
